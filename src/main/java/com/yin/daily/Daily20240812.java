 ...
 * 最多调用 100 次 search
 */
public class Daily20240812 {
    public static void main(String[] args) {
        MagicDictionary magicDictionary = new MagicDictionary();
        magicDictionary.buildDict(new String[]{"a"});
...
class MagicDictionary {
    ...
    public boolean search(String searchWord) {
        for (String item : ss) {
            int cnt = 0;
            for (int i = 0; item.length() == searchWord.length() && i < item.length() && cnt <= 1; i++) {
                if (item.charAt(i) != searchWord.charAt(i)) cnt++;
            }
